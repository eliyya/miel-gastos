...
export function SaleDialog({ products, sellers, today }: {
  ...
  return <div className="flex flex-wrap items-center gap-3">
    ...
    <Dialog.Root open={open} disablePointerDismissal onOpenChange={(value) => {
      ...
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-40 bg-primary/35 backdrop-blur-sm" />
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 flex max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border bg-card shadow-2xl">
          <div className="flex items-start justify-between gap-4 border-b px-5 py-4">
            <div><Dialog.Title className="font-serif text-2xl">Registrar venta</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Incluye todos los productos de la venta y revisa los importes antes de guardar.</Dialog.Description></div>
            ...
            <form className="min-h-0 overflow-y-auto" onSubmit={(event) => {
              ...
              <fieldset disabled={pending} className="space-y-5 p-5">
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <label className="grid gap-2 text-sm">Vendedor<NativeSelect name="sellerId" value={sellerId} onChange={(e) => setSellerId(e.target.value)} required><option value="">Selecciona un vendedor</option>{sellers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</NativeSelect></label>
                </div>
                <div className="space-y-3">
                  {rows.map((row, i) => <div key={row.key} className="grid grid-cols-2 items-end gap-3 rounded-xl border bg-background/40 p-4 sm:grid-cols-[minmax(0,1fr)_85px_100px_100px_40px]">
                    <label className="col-span-2 grid gap-2 text-sm sm:col-span-1">Producto {i + 1}<NativeSelect name="productId" value={row.productId} onChange={(e) => updateRow(row.key, { productId: e.target.value, unitPrice: undefined })} required><option value="">Selecciona un producto</option>{products.map((p) => <option key={p.id} value={p.id} disabled={rows.some((r) => r.key !== row.key && r.productId === p.id)}>{p.name}</option>)}</NativeSelect></label>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <label className="flex items-center gap-2 text-sm"><Checkbox name="paidByCard" checked={paidByCard} onCheckedChange={setPaidByCard} />Pago con tarjeta</label>
              ...
              <div className="sticky bottom-0 flex justify-end gap-2 border-t bg-card px-5 py-4">
