...
export function SalesTable({ sales }: { sales: SaleRow[] }) {
  ...
  return <Table>
    <TableHeader><TableRow>{["", "Fecha", "Cliente", "Vendedor", "Subtotal", "Pago vendedor", "Tarjeta", "Producción", "Costo", "Ganancia"].map((label, i) => <TableHead key={i} className={i >= 4 ? "text-right" : ""}>{label}</TableHead>)}</TableRow></TableHeader>
    <TableBody>{sales.map((sale) => {
      ...
      return <Fragment key={sale.id}>
        ...
        <TableRow id={`detail-${sale.id}`} hidden={!open} className="bg-muted/30 hover:bg-muted/30"><TableCell colSpan={10} className="px-6 py-4">
